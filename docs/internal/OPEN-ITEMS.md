...
All eleven tasks are done (167 tests, up from 136 when the branch started): the store-content value type and default-seed fingerprint, a raw-SQLite reader, candidate enumeration across all backup families, ranking and preselection, the offer rule, the launch-time restore, the prune fix, the `AppState` wiring, relaunch, and the picker UI.

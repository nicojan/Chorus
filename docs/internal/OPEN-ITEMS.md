...
## Done, not yet merged: the store recovery picker (branch `worktree-store-recovery-picker`)
...
Everything above is covered by the test suite. The user-facing behavior is not yet verified by hand — that pass is Task 10's Step 5 (now thirteen steps) and it belongs to the repo owner, since it means launching the app and manipulating files under `Application Support`. This branch changes recovery behavior that already shipped in 1.5.15, so it wants that pass before it ships, not just green tests. The shared-store-path item below is untouched by this branch and stays open.

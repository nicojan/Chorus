...
2. ~~`RailLayout` to two cases, with the `hybrid` forward-map and its test.~~ **Done with step 5 on 2026-08-17**, which is what the correction above asked for, so no hybrid user is moved twice. The forward-map was then seen working in the running app: the debug store had been left on `hybrid` the day before, and the rebuilt app drew it as one bar along the top.
